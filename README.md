...
cost surface generation script. The code to produce two loose sand maps (one for a large part of South Asia and a second for Europe and North Africa) is also provided as supplementary material.

   Consequently, the notebook calls the resulting maps from the author's GEE asset repository. The GEE code for the generation of these maps has also been provided for reproducibility reasons, but it is not required to run the notebook or the GEE code for the generation of cost surfaces.
